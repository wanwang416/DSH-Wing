...
describe("/workspace", () => {
  it("无参 → 显示当前工作区 + 用法", async () => {
    const ctx = { services: { workspace: { get: () => "本地目录", set: vi.fn() } } } as BridgeCommandContext;
    const res = await workspaceCommand.run(ctx, "", msg("/workspace"));
    expect(res?.text).toContain("本地目录");
  ...
  it("带有效路径 → 切换 + rotateSession 重建", async () => {
    ...
    const ctx = {
      services: { workspace: { get: () => "本地目录", set }, rotateSession },
    } as BridgeCommandContext;
    const res = await workspaceCommand.run(ctx, "本地目录/dsh-wing", msg("/workspace 本地目录/dsh-wing"));
    expect(set).toHaveBeenCalledWith("本地目录/dsh-wing");
